...
class Certificado(db.Model):
    ...
    horas_excedentes = db.Column(db.Integer, default=0)  # Novo campo para horas excedentes
...
def upload():
    ...
    if form.validate_on_submit():
        ...
        pontos, horas_excedentes = calcular_pontos(certificado_data)  # Obtenha os pontos e horas excedentes
        ...
        novo_certificado = Certificado(
            ...
            horas_excedentes=horas_excedentes  # Salva as horas excedentes
        )
...
def cursos():
    ...
    cursos_horas_excedentes = {qualificacao: 0 for qualificacao in QUALIFICACOES}

    # Somar os pontos e horas excedentes dos certificados aprovados do usuário logado
    for certificado in certificados_aprovados:
        cursos_pontos[certificado.qualificacao] += certificado.pontos
        cursos_horas_excedentes[certificado.qualificacao] += certificado.horas_excedentes

    # Criar a lista de cursos para passar para o template
    cursos_list = [{'nome': nome, 'pontos': pontos, 'horas_excedentes': cursos_horas_excedentes[nome]} for nome, pontos in cursos_pontos.items()]
